<div class="datemonthpicker_container">
    ...
    <div class="datemonthpicker_input">
        ...
        <div class="day_input" class:disabled={dayDropdownDisabled}>
            <Dropdown placeholder="Day" items={Days} currentitem={selectedDay} on:change={dayChangeHandler} disabled={dayDropdownDisabled} />
        </div>
    </div>
</div>
<script>
...
$: {
    if(value){
        ...
        if(initialMonth === 0){
            ...
            selectedMonth = Months.filter((m)=>m.value === initialMonth)[0];
        }
        if(initialDay === 0){
            selectedDay = null;
        }else{
...
function changeMonth(month){
    if(["January", "March", "May", "July", "August", "October", "December"].includes(month.label)){
        Days = calculateDays(31);
    }else if(["April", "June", "September", "November"].includes(month.label)){
