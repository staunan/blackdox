<header>
	<div class="page_title">Password Manager</div>
	<div class="menu_items">
		<SvelteButton title="Create New" on:tap={openCreateBookmarkModal}></SvelteButton>
	</div>
</header>
<div class="search_bookmark">
    <SearchBox placeholder="Search Bookmark" on:change={search_text_change}></SearchBox>
</div>
...
import {createBookmark, updateBookmark, getBookmarks, deleteBookmark, searchBookmark} from "apis/bookmark.js";
...
async function search_text_change(event){
    let formData = {
        'search_text': event.detail
    };
    try{
        let response = await searchBookmark(formData);
